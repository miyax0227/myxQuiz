...
app.service('qCommon', [
		...
		function($modal) {
			 ...
			 * @memberOf qCommon
			 * @param {Array.
			 *            <object>} order - key:比較対象属性名 order:昇順(asc)/降順(desc)
			 *            alter:置換文字列
			 * @param {boolean}
					...
					angular.forEach(order, function(record, i) {
						...
						if (this.flg == 0) {
							...
							if (record.hasOwnProperty('alter')) {
								aComp = qCommonService.decode(aComp,
										record.alter);
								bComp = qCommonService.decode(bComp,
										record.alter);
							}
							// 降順（desc）の場合
			...
			qCommonService.refreshCurrent = function(hist, scope) {
				...
				if (scope.current.players.length != hist.players.length) {
					scope.current.players.splice(0,
							scope.current.players.length);
					...
					angular.forEach(hist.players, function(record, i) {
						angular.forEach(scope.current.players[i], function(
								value, key) {
