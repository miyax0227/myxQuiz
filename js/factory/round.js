...
function(qCommon) {
   ...
   ************************************************************/
  {
   ...
   button_css : "btn btn-info",
   enable : function() {
   return (header.orderMode != "position");
   },
   action : function() {
   header.orderMode = "position";
   }
   }, {
   name : "priority",
   button_css : "btn btn-info",
   enable : function() {
   return (header.orderMode != "priority");
   },
   action : function() {
   header.orderMode = "priority";
   }
   }, {
   name : "judge-upper",
   button_css : "btn btn-danger",
  ...
  round.actions.map(function(action) {
    action.enable = function(player, scope) {
      return action.enable0(player, scope.current.players, scope.current.header);
    };
  ...
  round.global_actions.map(function(global_action) {
    if (angular.isUndefined(global_action.enable)) {
      global_action.enable = function(scope) {
        return global_action.enable0(scope.current.players, scope.current.header);
      };
    }
